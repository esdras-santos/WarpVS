...
if (!(0, fs_1.existsSync)('./tests/behaviour/solidity')) {
    (0, child_process_1.execSync)('bash ./tests/behaviour/setup.sh');
}
...
if (process.env.SM_BATCH) {
    ...
    (0, assert_1.default)(['1', '2', '3'].includes(process.env.SM_BATCH));
    filters = filters_batches[process.env.SM_BATCH];
}
else {
...
const whitelistData = (0, fs_1.readFileSync)(whitelistPath, 'utf-8');
function uncommentTests(filter) {
    (0, fs_1.writeFileSync)(whitelistPath, whitelistData
        .split('\n')
        .map((line) => {
...
filters.forEach((filter) => {
    ...
    console.log('------------------------------------------------------');
    try {
        (0, child_process_1.execSync)(`FILTER=${filter} npx mocha tests/behaviour/behaviour.test.ts --extension ts --require ts-node/register --exit`, { stdio: 'inherit' });
    }
    catch (e) {
        console.log(e);
        throw e;
...
(0, fs_1.writeFileSync)(whitelistPath, whitelistData);
//# sourceMappingURL=autoRunSemanticTests.js.map