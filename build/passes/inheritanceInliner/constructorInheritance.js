...
exports.solveConstructors = void 0;
...
/*
  ...
  ```
*/
function solveConstructors(node, ast, generator) {
    ...
    node.vLinearizedBaseContracts.forEach((contract) => {
        ...
        if (constructorFunc !== undefined)
            constructors.set(contract.id, constructorFunc);
    });
    ...
    const initFunctions = new Map(node.vLinearizedBaseContracts
        .map((contract) => [
        contract.id,
    ...
    // the previous step
    const visitedContracts = [node];
    node.vLinearizedBaseContracts.slice(1).forEach((contract) => {
        collectArguments(contract, visitedContracts, args, mappedVars, statements, selfConstructor.id, ast, generator);
    ...
    node.linearizedBaseContracts
        ...
        if (constructorFunc !== undefined) {
            ...
            (0, assert_1.default)(constructorFunc.vParameters.vParameters.length === argList.length, `Wrong number of arguments in constructor`);
            statements.push((0, nodeTemplates_1.createExpressionStatement)(ast, (0, functionGeneration_1.createCallToFunction)(newFunc, argList, ast)));
        }
    });
...
exports.solveConstructors = solveConstructors;
function collectArguments(contract, visitedContracts, args, idRemapping, statements, scope, ast, generator) {
    const constructorFunc = contract.vConstructor;
    if (constructorFunc !== undefined && constructorFunc.vParameters.vParameters.length > 0) {
        ...
        const argList = getArguments(callArgs, constructorFunc.vParameters.vParameters, idRemapping, statements, scope, ast, generator, contract);
...
function findConstructorCall(visitedContracts, currentContract, currentConstructor) {
    for (const contract of visitedContracts.slice().reverse()) {
        ...
        for (const specifier of contract.vInheritanceSpecifiers) {
            ...
            if (contractId === currentContract.id) {
                ...
                if (size > 0) {
                    (0, assert_1.default)(currentConstructor !== undefined &&
                        currentConstructor.vParameters.vParameters.length === size, 'Wrong number of arguments in constructor');
        ...
        if (constructorFunc !== undefined) {
            for (const modInvocation of constructorFunc.vModifiers) {
                ...
                if (contractDef instanceof solc_typed_ast_1.ContractDefinition && contractDef.id === currentContract.id) {
                    return modInvocation.vArguments;
                }
            }
        }
    }
    throw new errors_1.TranspileFailedError(`No arguments being passed to the constructor of ${(0, astPrinter_1.printNode)(currentContract)}`);
}
function getArguments(args, parameters, idRemapping, statements, scope, ast, generator, nodeInSourceUnit) {
    const size = args.length;
    const argList = [];
    for (let i = 0; i < size; ++i) {
        const newArg = (0, cloning_1.cloneASTNode)(args[i], ast);
        (0, utils_1.updateReferencedDeclarations)(newArg, idRemapping, idRemapping, ast);
        const newVar = (0, cloning_1.cloneASTNode)(parameters[i], ast);
        newVar.name = `__warp_constructor_parameter_${generator()}`;
        newVar.scope = scope;
        idRemapping.set(parameters[i].id, newVar);
        argList.push((0, nodeTemplates_1.createIdentifier)(newVar, ast, undefined, nodeInSourceUnit));
        statements.push(new solc_typed_ast_1.VariableDeclarationStatement(ast.reserveId(), '', [newVar.id], [newVar], newArg));
    }
    return argList;
}
function createFunctionFromConstructor(constructorFunc, node, ast, generator) {
    const newFunc = (0, cloning_1.cloneASTNode)(constructorFunc, ast);
    newFunc.kind = solc_typed_ast_1.FunctionKind.Function;
    newFunc.name = `__warp_constructor_${generator()}`;
    newFunc.visibility = solc_typed_ast_1.FunctionVisibility.Private;
    newFunc.isConstructor = false;
    newFunc.scope = node.id;
    ast.setContextRecursive(newFunc);
    return newFunc;
}
function generateBody(statements, constructorFunc, ast) {
    const newBody = (0, nodeTemplates_1.createBlock)(statements, ast);
    constructorFunc.vBody = newBody;
    ast.registerChild(newBody, constructorFunc);
}
function generateFunctionForConstructor(node, idRemapping, ast) {
    const newFunc = (0, nodeTemplates_1.createDefaultConstructor)(node, ast);
    ...
    if (currentConstructor !== undefined) {
        const parameters = currentConstructor.vParameters.vParameters.map((v) => {
            const newV = (0, cloning_1.cloneASTNode)(v, ast);
        ...
        newFunc.vParameters = (0, nodeTemplates_1.createParameterList)(parameters, ast);
        newFunc.vReturnParameters = (0, nodeTemplates_1.createParameterList)(retParameters, ast);
        newFunc.acceptChildren();
    }
    return newFunc;
}
function transformConstructor(node, newConstructor, statements, ast, generator) {
    const currentCons = node.vConstructor;
    if (currentCons === undefined)
        return;
    currentCons.kind = solc_typed_ast_1.FunctionKind.Function;
    currentCons.name = `__warp_constructor_${generator()}`;
    currentCons.visibility = solc_typed_ast_1.FunctionVisibility.Private;
    ...
    const argList = newConstructor.vParameters.vParameters.map((v) => {
        return (0, nodeTemplates_1.createIdentifier)(v, ast, undefined, node);
    });
    statements.push(new solc_typed_ast_1.ExpressionStatement(ast.reserveId(), '', (0, functionGeneration_1.createCallToFunction)(currentCons, argList, ast)));
}
function removeNonModifierInvocations(node) {
    const modifiers = node.vModifiers.filter((modInvocation) => {
        return modInvocation.vModifier instanceof solc_typed_ast_1.ModifierDefinition;
    });
    node.vModifiers = modifiers;
}
function removeModifiersFromConstructor(node) {
    const constructor = node.vConstructor;
    if (constructor !== undefined)
        removeNonModifierInvocations(constructor);
}
function findVarInitialization(contract) {
    const name = `${nameModifiers_1.INIT_FUNCTION_PREFIX}${contract.name}`;
...
function addCallToInitFunction(initFunctions, contractId, statements, ast) {
    const initFunc = initFunctions.get(contractId);
    if (initFunc !== undefined) {
        statements.push((0, nodeTemplates_1.createExpressionStatement)(ast, (0, functionGeneration_1.createCallToFunction)(initFunc, [], ast)));
    }
}
//# sourceMappingURL=constructorInheritance.js.map