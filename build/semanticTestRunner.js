...
if (!(0, fs_extra_1.existsSync)('./tests/behaviour/solidity')) {
    if (!(0, fs_extra_1.existsSync)('./tests/behaviour/setup.sh')) {
        new Error('setup.sh not found');
    }
    (0, child_process_1.execSync)(`VERSION=${version}./tests/behaviour/setup.sh`);
}
const whitelistPath = './tests/behaviour/expectations/semanticWhitelist.ts';
const whitelistData = (0, fs_extra_1.readFileSync)(whitelistPath, 'utf-8');
const testsSupportStatus = new Map(); // key: test-name , value : string
whitelistData.split('\n').forEach((line) => {
    ...
    if (trimmed.startsWith(`// 'tests/behaviour/solidity/test/libsolidity/semanticTests/`)) {
        ...
        if (testPathLets.length > 1) {
            testsSupportStatus.set(testPathLets[0].replace("'", '').replace(',', '').trim(), // remove possible trailing punctuation marks
...
if (process.argv.includes('--compare')) {
    ...
    const testFiles = (0, child_process_1.execSync)(
    ...
    console.log(`-------Tests which are present in solc version:${version} but not in whitelist------`);
    testFiles.forEach((file) => {
        if (!testsSupportStatus.has(file)) {
            console.log(file);
        }
    });
    console.log(`-------Tests which are present in whitelist but not in solc version:${version}------`);
...
// Test Execution Loop
const testDirs = (0, child_process_1.execSync)(
...
const currentTestsFileBody = (0, fs_extra_1.readFileSync)('./tests/behaviour/expectations/semanticTestsGenerated.ts', 'utf-8'); // Read current state of semanticTestsGenerated.ts
filterDirs.forEach((dir) => {
    const testFiles = (0, child_process_1.execSync)(`find ./${dir} -mindepth 1 -maxdepth 1 -name '*.sol'`)
        .toString()
        .trim()
        .split('\n')
        .sort()
    ...
    if (testFiles.length > 0) {
        ...
        (0, fs_extra_1.writeFileSync)('./tests/behaviour/expectations/semanticTestsGenerated.ts', testsFileBody.join('\n'));
        try {
            (0, child_process_1.execSync)(`FILTER=${dir} npx mocha tests/behaviour/behaviour.test.ts --extension ts --require ts-node/register --exit`, { stdio: 'inherit' });
        }
        catch (e) {
            console.log(e);
        }
...
(0, fs_extra_1.writeFileSync)('./tests/behaviour/expectations/semanticTestsGenerated.ts', currentTestsFileBody); // restore the content of the file
//# sourceMappingURL=semanticTestRunner.js.map